...
window = Tk()
window.title("Opsifin Autoprint")
...
branchLabel = Label(tabSetting, text="Branch")
...
checkBranch1 = Checkbutton(tabSetting, text='CCC Surabaya', variable=checkState1)
...
checkBranch2 = Checkbutton(tabSetting, text='CCC Balikpapan', variable=checkState2)
...
checkBranch3 = Checkbutton(tabSetting, text='CC', variable=checkState3)
...
checkBranch4 = Checkbutton(tabSetting, text='CCC 2', variable=checkState4)
...
checkBranch5 = Checkbutton(tabSetting, text='CCC 1', variable=checkState5)
...
checkBranch6 = Checkbutton(tabSetting, text='Global Corporate', variable=checkState6)
...
checkBranch7 = Checkbutton(tabSetting, text='CCC Makassar', variable=checkState7)
...
checkBranch8 = Checkbutton(tabSetting, text='Implant', variable=checkState8)
...
checkBranch9 = Checkbutton(tabSetting, text='Vaya Papua', variable=checkState9)
...
checkBranch10 = Checkbutton(tabSetting, text='Vaya HO', variable=checkState10)
...
textVal = StringVar()
text = Entry(tabSetting, width=10, textvariable=textVal)
...
def save():
    ...
    #save time schedule
    t = text.get()
    queryT = 'update time set timeSet=%s'%(t)
    c.execute(queryT)
    conn.commit()
    
    #save file path
    f = filePath.get()
    queryF = 'update path set filePath="%s"'%(f)
    c.execute(queryF)
    conn.commit()
...
button = Button(tabSetting, text="Save", command=save)
button.grid(column=1, row=8, sticky=W)
...
buttonFile.grid(column=1, row=6, sticky=W)

c.execute('SELECT filePath FROM path where id=1')
pVal = c.fetchone()[0]

filePath = StringVar()
file = Entry(tabSetting, width=35, textvariable=filePath, state='disabled')
file.grid(column=1, row=5, sticky=W)
filePath.set(pVal)
...
labelEndpoint = Label(tabAppRun, text='EndPoint')
labelEndpoint.grid(column=0,row=1, sticky=W)

endPoint = StringVar()
api = Entry(tabAppRun, width=40, textvariable=endPoint, state='disabled')
...
userAuth = StringVar()
user = Entry(tabAppRun, width=40, textvariable=userAuth, state='disabled')
user.grid(column=1, row=2, sticky=W)
userAuth.set(apiData[1])

labelPwd = Label(tabAppRun, text='Password')
labelPwd.grid(column=0,row=3, sticky=W)

pwdAuth = StringVar()
pwd = Entry(tabAppRun, width=40, textvariable=pwdAuth, state='disabled', show='*')
...
def editApi():
    ...
    buttonCancel = Button(tabAppRun, text="Cancel", command=cancelEdit)
...
buttonChangeApi.grid(column=0, row=4, pady=10, sticky=W)
